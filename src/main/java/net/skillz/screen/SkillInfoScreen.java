...
public class SkillInfoScreen extends Screen implements Tab {
    ...
    protected void init() {
        ...
        for (int i = 0; i < 50; i++) {
            Text skillExtraText = TextUtil.get("skill", this.skill.id().toString(), "lore", String.valueOf(i));
            if (skillExtraText.getString().contains(this.skill.id().toString()) && skillExtraText.getString().contains(String.valueOf(i))) {
                break;
            }
        ...
        if (this.lines.size() > skillInfoLines) {
            this.lines.add(skillInfoLines, new LineWidget(this.client, TextUtil.getGui("bonus.info"), null, 0));
        }

        addRestrictionLines(LevelManager.ITEM_RESTRICTIONS, TextUtil.get("restriction","item_usage"), 0);
        addRestrictionLines(LevelManager.BLOCK_RESTRICTIONS, TextUtil.get("restriction","block_usage"), 1);
        addRestrictionLines(LevelManager.ENTITY_RESTRICTIONS, TextUtil.get("restriction","entity_usage"), 2);
        addRestrictionLines(LevelManager.ENCHANTMENT_RESTRICTIONS, TextUtil.get("restriction","enchantments"), 3);
    }
    ...
    private void addRestrictionLines(Map<Integer, PlayerRestriction> levelRestrictions, Text restrictionText, int code) {
        ...
        for (Map.Entry<Integer, PlayerRestriction> itemRestriction : levelRestrictions.entrySet()) {
            // SkillId, Lvl
            for (Map.Entry<Identifier, Integer> specificRestriction : itemRestriction.getValue().getSkillLevelRestrictions().entrySet()) {
        ...
        for (Map.Entry<Integer, Map<Integer, PlayerRestriction>> restrictions : map.entrySet()) {
            this.lines.add(new LineWidget(this.client, TextUtil.getGui("level_short", restrictions.getKey()), null, code));
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawText(this.textRenderer, TextUtil.getGui("level_short", this.levelManager.getSkillLevel(this.skill.id())), this.x + 11 + this.textRenderer.getWidth(this.title), this.y + 7, 0x3F3F3F, false);
